    ...
    const saveSlip = async () =>{
       ...
       try {
        ...
        setSlipData({title:val.title,totalPrice,oneProduct:val.price,image:val.selectedFile}))   
         toast.success("Thank you for buying our products.Good Luck!")
  const data =  await axios.post(`${url}/user/saveSlip/${email}`,SlipData) 
  setopenfour(false)
       } catch (error) {
         console.log(error)
       }
...
<Box textAlign="left">
  ...
  <IconButton onClick={closeSlip} style={{marginLeft:"300px",marginTop:"-70px"}}><Close/></IconButton>
</Box>
...
<TableContainer component={Paper}>
      <Table aria-label="simple table">
        <TableHead color="primary">
          <TableRow>
            <TableCell style={{fontSize:"17px",color:"white",background:"#3F51B5"}}>Product Image</TableCell>
            <TableCell style={{fontSize:"17px",color:"white",background:"#3F51B5"}}>Product Price</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {state.map((row,index) => (
